...
function save_projs(filename, projections)
    proj_length = length(projections)
    println("projections length: $(proj_length)")

    k = 1
    cols = 3
    rows = ceil(Int, proj_length / cols)
    fig = Figure(size=(300cols, 400rows))
    @time for i in 1:rows, j in 1:cols
        k > proj_length && break

        proj_str = projections[k]
        println("projections $k: $(proj_str)")
        try
            ga = GeoAxis(
                fig[i, j];
                dest=proj_str,
                title="$k proj=$proj_str",
            )
            lines!(ga, coastlines)
        catch error
...
save_projs("support-projections-1-40.png", projections[1:40])
save_projs("support-projections-41-80.png", projections[41:80])
save_projs("support-projections-81-118.png", projections[81:118])